...
$pdf->Image("../../assets/img/logo_fci.png", 180, 10, 30, 30, 'PNG');
...
$pdf->SetTextColor(0, 0, 0);
$pdf->Cell(13, 5, utf8_decode('N°'), 0, 0, 'L');
$pdf->Cell(77, 5, utf8_decode('Descripción'), 0, 0, 'L');
$pdf->Cell(27, 5, 'Cantidad', 0, 0, 'L');
$pdf->Cell(27, 5, 'Precio', 0, 0, 'L');
$pdf->Cell(37, 5, 'Sub Total.', 0, 1, 'L');
...
while ($row = mysqli_fetch_assoc($ventas)) {
    $pdf->Cell(13, 5, $contador, 0, 0, 'L');
    $pdf->Cell(77, 5, $row['descripcion'], 0, 0, 'L');
    $pdf->Cell(27, 5, $row['cantidad'], 0, 0, 'L');
    $pdf->Cell(27, 5, $row['precio'], 0, 0, 'L');
    $pdf->Cell(37, 5, number_format($row['cantidad'] * $row['precio'], 2, '.', ','), 0, 1, 'L');
    $contador++;
    $total = $total + number_format($row['cantidad'] * $row['precio'], 2, '.', ',');
    #$pdf->Cell(196, 5, $total, 0, 1, 'R');
}
$pdf->SetTextColor(0, 0, 0);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(196, 5, 'Total pagar.', 1, 1, 'R');
$pdf->Cell(191, 5, number_format($total, 2, '.', ','), 0, 1, 'R');
